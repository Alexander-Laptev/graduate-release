            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 text-center">
                    ...
                    <form action="{{ route('record.date.store')}}" method="POST">
                        @csrf
                        <div class="my-4 grid gap-4 grid-rows-1 grid-flow-col w-full justify-center">
                            @foreach($dates as $date)
                                <div>
                                    @if(session('date_id') == $date->id)
                                        <button value="{{ $date->id }}" name="date_id" id="date_id" type="submit" class="bg-white rounded-[0.4375rem] p-2 border-2 border-purple-400">
                                    ...
                                    @if(session('date_id') != $date->id)
                                        <button value="{{ $date->id }}" name="date_id" id="date_id" type="submit" class="p-2">
                                            {{ $date->date->translatedFormat('j F') }}<br/>
                                            {{ $date->date->Format('Y') }}
                                        </button>
                        ...
                        <div class="text-start">
                            <div class="my-4 grid gap-4 grid-flow-row lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 w-full">
                                @foreach($hours as $hour)
                                    <div class="">

                                        <button id="multiLevelDropdownButton{{$hour['hour']}}" data-dropdown-toggle="dropdown{{$hour['hour']}}" class="w-full text-black bg-gradient-to-r from-purple-400 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-gray-300 rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">{{ $hour['hour'].':00' }} <svg class="w-4 h-4 ml-2" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>
                                        <!-- Dropdown menu -->
